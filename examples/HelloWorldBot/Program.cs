...
try
{
    Log.Information("Starting HelloWorldBot - Issues greeting example");
    ...
    // Load ProbotSharp apps from this assembly (will auto-discover HelloWorldApp)
    ...
    {
        application = "HelloWorldBot",
        description = "A simple bot that greets users when they open issues",
        version = "1.0.0",
        mode = "in-memory"
    }));
    ...
    Log.Information("HelloWorldBot started successfully");
    Log.Information("Webhooks endpoint: http://localhost:5000/webhooks");
    Log.Information("Health check: http://localhost:5000/health");
    Log.Information("Status endpoint: http://localhost:5000/hello-world/status");
    Log.Information("Running in IN-MEMORY mode - no persistence, no external dependencies");

    app.Run();

    Log.Information("HelloWorldBot stopped cleanly");
}
catch (Exception ex)
{
    Log.Fatal(ex, "HelloWorldBot terminated unexpectedly");
    throw;
}
finally
{
