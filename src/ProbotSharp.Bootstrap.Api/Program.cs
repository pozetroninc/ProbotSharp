...
try
{
    ...
    app.MapHealthChecks("/health", new HealthCheckOptions
    {
        ResponseWriter = async (context, report) =>
        {
            ...
            }, Program.HealthCheckJsonOptions);
        ...
        ILogger<Program> logger) =>
    {
        try
        {
            ...
            var webhookResult = await storage.GetAsync(DeliveryId.Create(deliveryId)).ConfigureAwait(false);
            ...
            // Enqueue for replay
            var enqueueResult = await replayQueue.EnqueueAsync(replayCommand).ConfigureAwait(false);
    ...
    {
        var result = await dlq.GetAllAsync().ConfigureAwait(false);
        if (!result.IsSuccess)
        {
    ...
    if (routeConfigurator != null)
    {
        ...
        await routeConfigurator.ConfigureRoutesAsync(app).ConfigureAwait(false);
    }
